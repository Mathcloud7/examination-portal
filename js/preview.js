...
if (["INPUT", "TEXTAREA", "SELECT", "BUTTON"].includes(ch.tagName))
return;
try {
...
qheader.innerHTML = `<div style="font-weight:700">Q${
  idx + 1
}</div><div class="small">Marks: ${escapeHtml(String(q.marks || 1))}</div>`;
...
escapeHtml(String(str || "").replace(/`/g, "‵").replace(/\$\{/g, "＄{"));
    const printable = ` <html> <head> <title>Print Preview</title> <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/katex@0.16.8/dist/katex.min.css"> <style>
...
.q h4{margin:0 0 8px 0;} </style> </head> <body> <h2>${safe(meta.title || "Assessment Preview")}</h2> <div>Class: ${safe(meta.theclass || meta.class || "")}</div> <div>Term: ${safe(meta.term || "")}</div> <div>Subject: ${safe(meta.subject || "")}</div> <hr/>
${questions
.map(
(q, idx) => `            <div class="q">               <div style="font-weight:700">Q${idx + 1}. ${safe(
                htmlToPlainText(q.question || "")
              )}</div>
              ${
                q.imageUrl
                  ?`<div style="margin-top:8px"><img src="${safe(
                   q.imageUrl
                ...
                ${(q.options || [])
                  .map(
                    (o, i) =>
                      `<div><strong>${String.fromCharCode(65 + i)}.</strong> ${safe(
htmlToPlainText(o || "")
)}</div>`
                  )
                  .join("")}               </div>             </div>`
...
const qInput = b.querySelector(
".questionInput, textarea, [data-field='text'], .editable"
);
const qText = qInput
...
```
  ...
  if (optionEls.length >= 1)
    options = Array.from(optionEls).map(
      o => (o.value ?? o.innerHTML ?? o.textContent || "").trim()
    );
  else {
  ...
  const answer = answerEl
    ? answerEl.value ?? answerEl.textContent ?? ""
    : b.querySelector(".correct")?.textContent || "";
